...
@export var module_type: Module.ModuleType = Module.ModuleType.NONE

var _placeable_count: int

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	if module_type == Module.ModuleType.NONE:
	...
	set_placeable_count(Module.max_module_count[self.module_type])
...
func set_placeable_count(new_count: int):
	self._placeable_count = clampi(new_count, 0, Module.max_module_count[self.module_type])
